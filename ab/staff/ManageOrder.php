...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['view'])){
    ...
    header("location:view_details.php?id=$order_id&type=$category");
}

// Fetch orders assigned to the logged-in staff member from the database
$staff_id = $_SESSION["STAFF"];
$sql = "SELECT orders.ORDER_ID, users.USER_ID, orders.SSIZE, orders.STATUSES, orders.CATEGORY, orders.ESTIMATED_DELIVERY_DATE 
        FROM orders
            ...
            <tbody>
                ...
                if ($result && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<button type='submit' name='view'>View Details</button>";
                        echo "</form>";
                        echo "</td>";
                        echo "</tr>";
                    }
